...
Jest configuration is defined in `jest.config.js`. To run the tests from the solution root, ensure Jest and `ts-jest` dev dependencies are installed (they are listed in `package.json`), then run:
